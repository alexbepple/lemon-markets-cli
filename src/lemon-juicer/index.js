...
const fetchOrders = (status) => () => {
    ...
    return got.get(resources.orders, reqOptions)
        .json()
        .then(r.prop('results'))
}
...
export const activateOrder = async (order) => {
    return got.put(r.join('/')([resources.orders, order.uuid, 'activate']) , defaultOptions)
}

export const fetchPortfolio = () =>
    got.get(resources.portfolio, defaultOptions)
        .json()
        .then(r.prop('results'))
